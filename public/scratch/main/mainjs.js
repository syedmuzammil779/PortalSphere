...
if (!window.cartService) {
  window.cartService = {
     ...
     *
     * @param {Object} result - The JSONified data received from the API after making a cart updated
     */
    updateCartState: function (result) {
      const $ = window.jQuery
      if (result.sections && result.sections['cart-drawer']) {
        const tempContainer = $('<div></div>').html(result.sections['cart-drawer'] + result.sections['header']);
        const newCartDrawerInner = tempContainer.find('.drawer__inner');
        const currentCartDrawer = $('cart-drawer');

        if (newCartDrawerInner && currentCartDrawer) {
          currentCartDrawer.removeClass('is-empty');
          const cartDrawerInner = currentCartDrawer.find('.drawer__inner');
          if (cartDrawerInner) {
            cartDrawerInner.html(newCartDrawerInner.html());
          }
          else {
            currentCartDrawer.append(newCartDrawerInner);
          }
        }

        const newHeader = tempContainer.find('header');
        const newCartCountBubble = newHeader.find('.cart-count-bubble');
        const currentHeader = $('header').first();
        const cartIconBubble = currentHeader.find('#cart-icon-bubble');
        const newCartIconBubble = newHeader.find('#cart-icon-bubble');
        const newCartIcon = newCartIconBubble.find('.svg-wrapper');
        const cartIcon = cartIconBubble.find('.svg-wrapper');
        const cartCountBubble = currentHeader.find('.cart-count-bubble');

        if (newCartCountBubble && currentHeader) {
          if (cartCountBubble) {
            cartIcon.html(newCartIcon.html());
            cartCountBubble.html(newCartCountBubble.html());
          }
          else {
            if (cartIconBubble) {
              cartIconBubble.append(newCartCountBubble);
            }
          }
        }
        if (currentCartDrawer[0]) {
          currentCartDrawer[0].open();
        }
      }
    }
  }
