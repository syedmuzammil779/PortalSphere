...
if (!window.cartService) {
  window.cartService = {
    addProductToCart: async function (variantId, quantity, properties = {}) {
      try {
        const currCartState = await this.getCart()
        console.debug('Adding variant to cart:', variantId, 'Quantity:', quantity);
        const result = await this.handleCartRequest('CREATE', {
          items: [{
            id: variantId,
            quantity: quantity,
        ...
        const newCartState = await this.getCart()

        console.debug('Product added to cart:', result);

        await this.updateCartState(currCartState, newCartState)
    ...
    updateProductToCart: async function (variantId, quantity, properties = {}) {
      try {
        ...
        const result = await this.handleCartRequest("UPDATE", {
          id: variantId,
          quantity: quantity,
          properties: properties,
          sections: ['cart-template', 'header']
        ...
        await this.updateCartState(currCartState, result)
    ...
    getCart: async function () {
      try {
        const cart = await this.handleCartRequest("READ");
     ...
     *
     * @param {string} type - Enum that takes "CREATE", "READ" and "UPDATE". Any other types will throw an error
     * @param {Object} body - Request body JSON object. Null by default. DO NOT STRINGIFY.
     *
     * @returns {Object} response
     */
    handleCartRequest: async function (type, body = null) {
      let method = ""
      let url = ""
      ...
      // only add the body if the sending a POST request
      if (method == "POST") {
    ...
    /** finds the difference between the old and new cart
     *
     * @param {Object} oldCart - The JSONified data received from the API before making a cart update
     * @param {Object} newCart - The JSONified data received from the API after making a cart update
     *
     */
    diffCart: function (oldCart, newCart){
     ...
     *
     * @param {Object} prev - The JSONified data received from the API before making a cart update
     * @param {Object} result - The JSONified data received from the API after making a cart update
     */
    updateCartState: async function (prev, result) {
      $("li.cart a[href='#cart'] span").text(result.item_count);
      if (prev.item_count === 0 || result.item_count === 0) {
        modCartDrawer();
        return
      }
      $("div.subtotal-row-price span.subtotal-total-price").text((result.total_price / 100).toFixed(2))
      const { added, updated, deleted } = this.diffCart(prev, result)
      added.forEach(async (add) => {
        ...
        createCartItem(add, volumeData, $("#cart div.custom-cart-content"), function (data) {})
      });
      updated.forEach(update => {
        const cartItemEl = $(`#cart div[data-cart-product='${update.handle}']`)
        cartItemEl.find("span.subtotal-el-price").text((update.final_line_price / 100).toFixed(2));
        cartItemEl.find("span.subtotal-price-with-curr").css("display", "flex")
        cartItemEl.find("span.subtotal-price-loading").css("display", "none")
        cartItemEl.find("div.cart-item-data span.discount-desc").text(update.discounts[0].title)
      });
      deleted.forEach(delItem => {
        $(`#cart div[data-cart-product='${delItem.handle}']`).remove()
      })
    },
  }
}
...
if (!window.loginRegisterService) {
  window.loginRegisterService = {
    ...
    createLoginRegisterButtons: function () {
      ...
      complement.complementaryProductBlock = $('#complementary-product-block');
      complement.complementaryProductBlock.html(element);
      complement.complementaryProductLoginButtons = $('#complementary-product-login-buttons');
      complement.complementaryProductRegisterButton = $('#complementary-product-register-button');
    }
  }
}
  ...
  window.productPageService = {
    createProductPageCustomPricing: function () {
      if (!window.productPageState.productOriginalFlag) {
        ...
        window.productPageState.new.productQuantityMinus = $('#product-quantity-minus');
        window.productPageState.new.productQuantityPlus = $('#product-quantity-plus');
        window.productPageState.new.productQuantityInput = $('#product-quantity-input');
        window.productPageState.new.productQuantityElement = $('#product-quantity-input-block');
        ...
        window.productPageState.new.productPriceElement = $('#product-price');
        window.productPageState.new.productLoadingSpinner = $('#product-loading-spinner');
      }
    },
...
if (!window.complementaryProductService) {
  window.complementaryProductService = {
    ...
    getComplementaryProductWithConfig: async function (appDomain, shopDomain, apiKey, timestamp, hmac, productId, variantId, storeType) {
      try {
        let data = null;
        const response = await fetch(`https://${appDomain}/api/complementary-products-with-config?shop=${shopDomain}&productId=${productId}&variantId=${variantId}&api_key=${apiKey}&timestamp=${timestamp}&hmac=${hmac}&storeType=${storeType}`);
        data = await response.json();
        console.debug('Complementary Product Service:', data);
        return data;
      } catch (error) {
