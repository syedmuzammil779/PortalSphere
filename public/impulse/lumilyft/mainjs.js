//Init Interceptor
// Set up global event tracking FIRST (before any other scripts)
...
if (!window.GlobalVariables.isGlobalClickEventAdded ) {
  ...
  // const btnAddToCart = document.querySelector('button[name="add"][type="submit"]');
  //   if (btnAddToCart) {  
  ...
  //        // window.eventInterceptor(async (event) => {
  //      //     alert("my order")
  //       //    delay(window.cartService.updateProductToCart(existingItem.key, existingItem.quantity , existingItem.properties),1500);
  //      //   }, true, btnAddToCart, 'click');
  ...
  //         console.debug("Extracted Variant ID:", variantIdNew);
  ...
  window.productPageService = {
    createProductPageCustomPricing: function() {
      if (!window.productPageState.productOriginalFlag) {
        ...
        window.productPageState.new.productVariantSelector = document.getElementsByClassName('variant-input-wrap')[0]
      }
    },
          ...
          <div style="padding: 20px; height: 100%; display: flex; flex-direction: column;">
            ...
            <div style="flex-grow: 1; overflow-y: auto; scrollbar-width: none; -ms-overflow-style: none;">
              ...
              <div style="margin-bottom: 20px;">
                <div style="display: flex; width: 10%; justify-content: center; align-items: center; margin-bottom: 20px;">
            ...
            <div style="padding: 20px; border-top: 1px solid #eee; background: white;">
              ...
              <button id="cp-ts-add-to-cart" style="width:10% !important; background-color: #000; color: white; border: none; padding: 15px; width: 100%; font-size: 16px; cursor: pointer; margin-bottom: 10px;">Add</button>
              <button id="close-top-seller-nothanks" style="width: 10% !important; background: none; border: none; color: #666; cursor: pointer; width: 100%;">No thank you</button>
            </div>
          </div>
        </div>
      `;
...
// Helper: Render cart items HTML from /cart.js data (simple version, you can expand as needed)
function renderCartItems(cart) {
  if (!cart.items || cart.items.length === 0) {
    return '<div class="drawer__cart-empty appear-animation appear-delay-2"><div class="drawer__scrollable">Your cart is currently empty.</div></div>';
  }
  let html = `<div class="cart__items" data-count="${cart.items.length}" data-cart-subtotal="${cart.total_price}">`;
  for (const item of cart.items) {
    html += `
      <div class="cart__item" data-key="${item.key}">
        <div class="cart__image">
          <a href="${item.url}" class="image-wrap loaded">
            <img src="${item.image}" alt="${item.product_title}" width="80" loading="lazy" class="image-element" />
          </a>
        </div>
          ...
          <div class="cart__item-sub">
            <div>
              <div class="js-qty__wrapper">
                ...
                <input type="text" name="updates[]" class="js-qty__num" value="${item.quantity}" min="0" pattern="[0-9]*" data-id="${item.key}">
                <button type="button" class="js-qty__adjust js-qty__adjust--minus" aria-label="Reduce item quantity by one">-</button>
                <button type="button" class="js-qty__adjust js-qty__adjust--plus" aria-label="Increase item quantity by one">+</button>
              </div>
              <div class="cart__remove">
                <a href="/cart/change?id=${item.key}&quantity=0" class="text-link">Remove</a>
              </div>
            </div>
            <div class="cart__item-price-col text-right">
              <span class="cart__price cart__discount">${Shopify.formatMoney ? Shopify.formatMoney(item.final_line_price) : (item.final_line_price / 100).toFixed(2)}</span>
            </div>
          </div>
        </div>
      </div>
...
function refreshCartDrawerAndCountNonDestructive() {
  fetch('/cart?view=ajax')
    .then(res => res.text())
    .then(html => {
      console.log('[CartDrawer Debug] Fetched /cart?view=ajax HTML:', html);
      const tempDiv = document.createElement('div');
      tempDiv.innerHTML = html;
      const newCartDrawer = tempDiv.querySelector('#CartDrawer');
      const oldCartDrawer = document.getElementById('CartDrawer');
      console.log('[CartDrawer Debug] newCartDrawer:', newCartDrawer);
      console.log('[CartDrawer Debug] oldCartDrawer:', oldCartDrawer);
      if (newCartDrawer && oldCartDrawer) {
        oldCartDrawer.replaceWith(newCartDrawer);
        newCartDrawer.classList.add('drawer--is-open');
        newCartDrawer.setAttribute('tabindex', '-1');
        console.log('[CartDrawer Debug] CartDrawer replaced and kept open.');
      } else {
        console.warn('[CartDrawer Debug] Could not find #CartDrawer in new or current DOM.');
      }
    });
}
      ...
      <div id="topseller-product-block" style="padding: 10px; position: relative; background-color: #f8faf9;">        
        ...
        <div id="top-product-details" style="display: flex; align-items: center; gap: 15px; margin-bottom: 15px;">
          ...
          <div id="top-quantity-input-group" style="border: 1px solid #ddd; border-radius: 4px; display: flex; align-items: center;">
            <button type='button' id="top-quantity-minus" style="width: 32px; height: 32px; background: none; border: none; border-right: 1px solid #ddd; font-size: 16px; cursor: pointer; padding: 0;">−</button>
            ...
            <button type='button' id="top-quantity-plus" style="width: 32px; height: 32px; background: none; border: none; border-left: 1px solid #ddd; font-size: 16px; cursor: pointer; padding: 0;">+</button>
          </div>
        </div>  
