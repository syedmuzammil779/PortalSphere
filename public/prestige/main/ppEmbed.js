...
const productCardHeadingSelectors = [
    '.product-card__info .product-title' //parallax theme
];
...
(async function () {
    ...
    if ((isB2B && isCustomerLoggedIn && customerTag == null) || (isHybrid && isCustomerLoggedIn && customerTag == null) ) {
        ...
        if (currentPage.includes('/products/')) {      // display login register buttons
            ...
            const stickyAddToCart = document.querySelector('product-sticky-bar');
            if(stickyAddToCart) {
    ...
    if(customerTag) {
        ...
        const hoverButtons = document.querySelectorAll('button.product-card__quick-add-button');
            ...
            if(data != null && data.length > 0) {
                productsCards.map(el => {
                    ...
                    if(matchingResp && matchingResp.returnData) {
                        ...
                        const priceSelector = el.cardElement.querySelector('price-list');
                        if(priceSelector) {
                            ...
                            const imageSelector = el.cardElement.querySelector('.product-card__figure');
                            if(imageSelector) {
                                const onSaleBadge = imageSelector.querySelector('.badge--on-sale');
                                if(onSaleBadge) {
                                    ...
                                    imageSelector.insertAdjacentHTML('beforeend', `
                                        <badge-list class="badge-list badge-list--vertical">
                                            <on-sale-badge class="badge badge--on-sale">
                                                Save ${firstPriceConfig.percentage}%
                                            </on-sale-badge>
                                        </badge-list> 
                                    `);
                                }
...
function extractProductHandle(card) {
    const anchorTag = card.querySelector('a.product-title');
    if(anchorTag) {
