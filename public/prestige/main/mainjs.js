  ...
  window.cartService = {
    ...
    populateCartDrawer: async function(result) {
      if (result.sections && result.sections['cart-drawer']) {
        ...
        tempContainer.innerHTML = result.sections['cart-drawer']
    ...
    openCartDrawer: function () {
      const nonExpandedButton = document.querySelector('a[aria-controls="cart-drawer"][aria-expanded="false"]');
      if(nonExpandedButton) {
        nonExpandedButton.click();
      } else {
        const cartButton = document.querySelector('a[aria-controls="cart-drawer"]');
        if(cartButton) {
          cartButton.click();
        }
      }
...
if (!window.GlobalVariables.isGlobalClickEventAdded ) {
  ...
  if(currentPage.includes('/products/')) {
    if (window.productPageState && Object.hasOwn(window.productPageState, 'skipEvent') && window.productPageState.skipEvent === true) {
      ...
      const formSelector = document.querySelector('product-form');
      if(formSelector) {
        const formElement = formSelector.querySelector('form[action="/cart/add"]');
        if(formElement) {
          formElement.querySelector('button[type="submit"]').addEventListener('click', async function (event) {
            ...
            const variantId = formSelector.querySelector('[name="id"]').value;
        ...
        const variantPickerSelector = document.querySelector('div[data-block-id="variant_picker"]');
        if(variantPickerSelector) {
          const variantPickerEl = variantPickerSelector.querySelector('variant-picker');
          if(variantPickerEl) {
            variantPickerEl.addEventListener('click', function () {
              setTimeout(() => {
                location.reload();
              }, 500);
            });
    ...
    createProductPageCustomPricing: function() {
      if (!window.productPageState.productOriginalFlag) {
        ...
        const priceElement = `
          ...
          <div id="product-price" style="display: none; margin: 0 0 10px;"></div>
        `;
