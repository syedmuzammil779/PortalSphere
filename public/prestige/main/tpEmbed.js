...
async function addTopSellerToCart(variantId, quantity) {
  try {
    ...
    if (existingItem) {
      ...
      cartResponse = await window.cartService.updateProductToCart(lineItemId, existingItem.quantity + quantity, {
        "_isUpsellOrigin": existingIsUpsellOrigin,
        "_upsellQuantity": parseInt(existingUpsellQuantity) + quantity
      });
    } else {
      cartResponse = await window.cartService.addProductToCart(variantId, quantity, {
...
function insertTopSeller() {
  ...
  if (cartDrawer && !cartDrawer.querySelector('#top-product-embed')) {
    const drawerInner = cartDrawer.querySelector('.cart-drawer__items');
    if (drawerInner) {
      const topSellerDiv = createTopSellerElement();

      drawerInner.insertAdjacentElement('afterend', topSellerDiv);
      ...
      topProductState.addToCartTopProduct.onclick = async (e) => {
        ...
        setTimeout(async () => {
          ...
          const cartResponse = await addTopSellerToCart(variantId, quantity);
          document.querySelector('dialog-close-button button').click();
          await window.cartService.populateCartDrawer(cartResponse);
          await window.cartService.openCartDrawer();
        }, 500);
      };
...
function enableCartButtons() {
  const quantityButtons = document.querySelectorAll('cart-drawer a.quantity-selector__button');
...
function checkCartDrawerAndToggle() {
  ...
  if (cartDrawer) {
    ...
    if (savingsbadge.length === 0) {
      disableCartButtons();
      updateCartItemsUpsell(); //Break it into two functions, one for dealing with cart + and - buttons and one for savings badges
      enableCartButtons();
    }
  } else {
    removeTopSeller();
...
const setInputConstraints = (input, config) => {
  ...
  input.setAttribute('step', config.volumeConfig.increment);
  input.setAttribute('min', config.volumeConfig.minimum);
  input.setAttribute('max', config.volumeConfig.maximum);
};

async function updateCartItemsUpsell() {
    const rows = document.querySelectorAll('line-item.line-item');
    topProductState.cartItemsVariants = Array.from(rows).map(row => {
      const href = row.querySelector('a.h6')?.href;
      if (!href) return null;
    ...
    for (let i = 0; i < topProductState.cartItemsVariants.length; i++) {
      ...
      const allButtons = input.parentElement.parentElement.querySelectorAll('a.quantity-selector__button');
      if(allButtons) {
        allButtons.forEach(el => {
          if(!el.hasAttribute('data-set')) {
            const url = new URL(el.href, window.location.origin);
            const spanTag = el.querySelector('span.sr-only');
            if(spanTag && spanTag.textContent.includes('Decrease')) {
              const currentQty = parseInt(input.value, 10);
              const newQty = Math.max(currentQty - parseInt(data.volumeConfig.increment), data.volumeConfig.minimum);
              ...
              el.addEventListener('click', async function (e) {
                ...
                const cart = await addTopSellerToCart(variantId, newQty - input.value);
                input.value = newQty;
                await window.cartService.populateCartDrawer(cart);
                setTimeout(() => {
            ...
            if(spanTag && spanTag.textContent.includes('Increase')) {
              const currentQty = parseInt(input.value, 10);
              const newQty = Math.min(currentQty + parseInt(data.volumeConfig.increment), data.volumeConfig.maximum);
              url.searchParams.set("quantity", newQty);
              ...
              el.addEventListener('click', async function (e) {
                ...
                e.target.style.disabled = true;
                input.value = newQty;
                const cart = await addTopSellerToCart(variantId, parseInt(data.volumeConfig.increment));
                await window.cartService.populateCartDrawer(cart);
                setTimeout(() => {
                  window.cartService.openCartDrawer();
                }, 500);
      ...
      savingsButton.onclick = async (e) => {
        ...
        const cartResponse = await addTopSellerToCart(newVariantId, diff, properties);
        await window.cartService.populateCartDrawer(cartResponse);
