    ...
    addProductToCart: async function(variantId, quantity, properties = {}) {
      try {
        ...
        if (result.sections && result.sections['cart-drawer']) {
          ...
          const newCartDrawerInner = tempContainer.querySelector('.drawer__panel');
          const currentCartDrawer = document.querySelector('#MiniCart-cart-drawer');
          if (newCartDrawerInner && currentCartDrawer) {
            currentCartDrawer.classList.remove('is-empty');
            const cartDrawerInner = currentCartDrawer.querySelector('.drawer__panel');
            if (cartDrawerInner) {
...
if (!window.GlobalVariables.isGlobalClickEventAdded ) {
  ...
  const variantPickerSelector = document.querySelector('variant-picker');
    ...
    createProductPageCustomPricing: function() {
      if (!window.productPageState.productOriginalFlag) {
        ...
        const priceElement = `
          ...
          <div id="product-price" class="product__price" style="display: none;"></div>
        `;
