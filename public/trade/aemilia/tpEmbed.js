...
async function recordButtonClick(buttonType, tag, operation) {
  var buttonClickResponse = await window.customerService.recordButtonClick(tpConfig.appDomain, tpConfig.shopDomain, tpConfig.apiKey, tpConfig.timestamp, tpConfig.hmac, tpConfig.customerId, tag, buttonType, operation);
...
async function updateCartItemsUpsell() {
    ...
    for (let i = 0; i < topProductState.cartItemsVariants.length; i++) {
        ...
        var removeButtonEvent = false;
        const inputParentSelectorForDelete = input.parentElement.parentElement.parentElement;
        if(inputParentSelectorForDelete) {
          const removeButton = inputParentSelectorForDelete.querySelector('button.cart-remove-button');
          if(removeButton) {
            if(!removeButtonEvent) {
              removeButton.addEventListener('click', async () => { 
                setTimeout(() => {setOptionsOnTheProductPageDom()}, 2000);
              })
              removeButtonEvent = true;
            }
          }
        }
