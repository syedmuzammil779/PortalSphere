...
if (!window.cartService) {  
  window.cartService = {
    ...
    getCart: async function () {
      try {
        const response = await fetch('/cart.js?sections=ajaxcart', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json'
          }
...
if (!window.GlobalVariables.isGlobalClickEventAdded ) {
  ...
  setTimeout(async () => {    
    document.querySelector('button[name="add"]').addEventListener('click', async function (e) {
      e.preventDefault();
      ...
      if(lineItems.length === 0) {
        const topSellerEl = await window.topSellerPopupService.createTopSellerPopup();
        document.querySelector('.js-drawer-open-right-link').click();
        
        setTimeout(async () => {
          document.querySelector('p.empty-cart').innerHTML = '';
          document.querySelector('.cart--continue-shopping').parentElement.remove();
          document.querySelector('.drawer__header').remove();
          document.getElementById('CartContainer').appendChild(topSellerEl);
  ...
  if(window.location.pathname.includes('products')) {
    setTimeout(async () => {    
      document.querySelector('.form-vertical').addEventListener('click', function(event) {
        if (event.target.localName.toLowerCase() === 'label') {
          if(event.target.parentElement.classList.contains('swatch-element')) {
            setTimeout(() => {
              location.reload();
            }, 500);
          }      
...
if (!window.loginRegisterService) {
  window.loginRegisterService = {
    ...
    createLoginRegisterButtons: function() {
      ...
      document.querySelector('.product__meta').insertAdjacentHTML('afterend',element);
      ...
      if (!window.productPageState.productOriginalFlag) {
        const qtyElement = `
          <div id="product-quantity-input-block" style="display: flex; align-items: center; border: 1px solid #ccc; border-radius: 4px; width: fit-content;">
            ...
            <input id="product-quantity-input" style="padding: 0; margin: 0; width: 40px; height: 40px; border: none; font-size: 16px; text-align: center;" value="1" readonly>
            ...
            <div style="flex-grow: 1; overflow-y: auto; scrollbar-width: none; -ms-overflow-style: none;">
              ...
              <div style="margin-bottom: 20px;">
                <div style="display: flex; justify-content: center; align-items: center; margin-bottom: 20px;">
                  ...
                  <img style="display: none; max-width: 100%; height: 200px;" id="cp-ts-product-image" src="">
                </div>
  ...
  addTopSellerToCart: async function(variantId, quantity) {
    try {
      ...
      if (existingItem) {
        ...
        returnVal = await window.cartService.updateProductToCart(lineItemId, existingItem.quantity + quantity, {
          "_isUpsellOrigin": existingIsUpsellOrigin,
          "_upsellQuantity": parseInt(existingUpsellQuantity) + quantity
        });
      } else {
        // Add new item
        returnVal = await window.cartService.addProductToCart(variantId, quantity, {
