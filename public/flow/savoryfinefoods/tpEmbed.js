...
function createTopSellerElement() {
    ...
    topSellerDiv.style.margin = '20px 100px 20px 100px'
    ...
    try {
        ...
        if (existingItem) {
          // Update existing line item
            const lineItemId = existingItem.key.split(':')[0];
        ...
        console.error('Error adding/updating addTopSellerToCart product in cart:', error);
        showComplementaryToast('Failed to add addTopSellerToCart product to cart. Please try again.', 'error');
    }
}
...
function insertTopSeller() {
    ...
    if (cartDrawer && !cartDrawer.querySelector('#top-product-embed')) {
        const drawerInner = cartDrawer.querySelector('.ajaxcart__inner');
        if (drawerInner) {
            const topSellerDiv = createTopSellerElement();
            
            const drawerFooter = drawerInner.querySelector('.ajaxcart__footer-wrapper');
            if (drawerFooter) {
...
async function updateCartItemsUpsell() {
  const rows = document.querySelectorAll('.ajaxcart__row')
  topProductState.cartItemsVariants = Array.from(rows).map(row => {
    const href = row.querySelector('a.ajaxcart__product-name')?.href;
    if (!href) return null;
    return href.split('variant=')[1];
  }).filter(id => id);

  // get quantity controls
  const quantityInputs = document.querySelectorAll('.ajaxcart__qty');
  topProductState.cartQuantityInputs = Array.from(quantityInputs).map(container => {
    return container.querySelector('input.ajaxcart__qty-num');
  }).filter(input => input);
  ...
  for (let i = 0; i < topProductState.cartItemsVariants.length; i++) {
    ...
    const setInputConstraints = (input, config, variantId) => {
      input.readOnly = true;
      ...
      const minusButton = input.parentElement.querySelector('.ajaxcart__qty--minus');
      const plusButton = input.parentElement.querySelector('.ajaxcart__qty--plus');
      ...
      if(minusButton && plusButton) { 
        minusButton.addEventListener('click', async function (e) {
          ...
          var newValue = value - parseInt(config.volumeConfig.increment);
          if(newValue < parseInt(config.volumeConfig.minimum)) {
            newValue = parseInt(config.volumeConfig.minimum);
          } else {
            input.value = newValue;
            await window.cartService.updateProductToCart(variantId, input.value);
            window.reloadPageWithCartOpen('openCart', 'true');
          }
        });
        ...
        plusButton.addEventListener('click', async function (e) {
          ...
          e.stopImmediatePropagation();
          const value = parseInt(input.value);
          const increment = parseInt(config.volumeConfig.increment);
    ...
    setInputConstraints(input, volumeConfig, variantId);
    ...
    input.addEventListener('change', async () => {
      ...
      try {
        ...
        setInputConstraints(input, updatedConfig, variantId);
    ...
    savingsButton.onclick = async (e) => {
      ...
      const inputForThisRow = e.target.parentElement.querySelector('.ajaxcart__qty-num');
      if(inputForThisRow) {
        inputForThisRow.value = parseInt(inputForThisRow.value) + parseInt(diff);
        window.reloadPageWithCartOpen('openCart', 'true');
      }
