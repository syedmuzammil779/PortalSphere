...
(function () {
    ...
    function waitForProductPricingService(callback, retries = 10, delay = 300) {
        const interval = setInterval(() => {
            if (
                window.productPricingService &&
                typeof window.productPricingService.getVolumePricingByProductVariantId === 'function'
            ) {
                clearInterval(interval);
                callback(); // Now it's safe to call it
            } else if (retries <= 0) {
                clearInterval(interval);
                console.error('productPricingService did not load in time.');
            } else {
                retries--;
            }
        }, delay);
    ...
    async function populateVolumePricingTable() {
        ...
        try {
            let volumePricingData = await window.productPricingService.getVolumePricingByProductVariantId(
                pptConfig.appDomain,
                shop,
                pptConfig.apiKey,
                pptConfig.timestamp,
            ...
            priceConfig.forEach(config => {
                ...
                if (volumePricingData.type === 'fixedAmount') {
                    discountCell.textContent = `${config.currencySymbol}${fixDecimals(config.discountAmount)}`;
                } else {
                    discountCell.textContent = `${config.percentage}%`;
                }
    ...
    // Wait for productPricingService before calling populate
    waitForProductPricingService(populateVolumePricingTable);
})();
