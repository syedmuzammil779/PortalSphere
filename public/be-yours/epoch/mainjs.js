...
async function toggleCartDrawerQuantityInteractivity() {
  const topProductEmbed = document.getElementById('top-product-embed');
  const isTopProductVisible = isElementEffectivelyVisible(topProductEmbed);
  const disabled = !isTopProductVisible;
  bindCartDrawerQuantityButtons();

  console.log('Top product embed content:', topProductEmbed?.innerHTML.trim());
  console.log('Is Top Product Visible?', isTopProductVisible);
  ...
  for (const input of quantityInputs) {
    ...
    try {
      const volumeApiConfig = await getVolumePricingConfig(variantId);
      if (!volumeApiConfig) continue;
      ...
      input.setAttribute('data-min', minNum);
      input.setAttribute('data-max', maxNum);
      ...
      // Attach listeners to enforce min, max, step
      input.addEventListener('change', () => {
        let value = parseInt(input.value, 10);

        if (isNaN(value)) value = minNum;
        if (value < minNum) value = minNum;
        if (value > maxNum) value = maxNum;

        const remainder = (value - minNum) % stepNum;
        if (remainder !== 0) value = value - remainder;

        input.value = value;

        if (typeof updateProductToCart === 'function') {
          updateProductToCart(variantId, value);
        }
      });
    } catch (e) {
      console.error(`Failed to apply volume pricing for variant ${variantId}:`, e);
    }
  }
  ...
  window.productPageService = {
    createProductPageCustomPricing: function() {
      if (!window.productPageState.productOriginalFlag) {
        ...
        const oldQuantityElement = document.querySelector('.product__info-container .price');
        ...
        if (!document.getElementById('product-quantity-input-block')) {
          ...
          const insertAfterElement = document.querySelector('.form') || document.querySelector('.shopify-product-form');
...
function updateCartCount() {
  fetch('/cart.js')
    ...
    .then(cart => {
      ...
      // Optional: Update screen reader text or hidden span (if used)
      ...
      if (bubble) {
        ...
        const sr = bubble.querySelector('span.visually-hidden');

        if (visible) visible.textContent = count;
        if (sr) sr.textContent = `${count} item${count !== 1 ? 's' : ''}`;
      }
    })
    .catch(error => {
      console.error('[Cart Count] Failed to update:', error);
    });
}
...
async function updateProductToCart(variantId, quantity) {
  ...
  try {
    ...
    const cart = await response.json(); // ✅ Read JSON once

    if (!response.ok) {
      throw new Error(cart.description || 'Failed to update cart');
    }

    // ✅ Optional: Re-validate quantity min/max here if needed

    await updateCartSidebar?.();
    await updateCartCount?.();

    // ✅ Call interactivity reset to refresh min/max/step values
    if (typeof toggleCartDrawerQuantityInteractivity === 'function') {
      toggleCartDrawerQuantityInteractivity();
    }
    ...
    console.error('[Cart] updateProductToCart error:', error);
