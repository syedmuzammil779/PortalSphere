...
function insertTopSeller(cartDrawer) {
  if (cartDrawer && !cartDrawer.querySelector('#top-product-embed')) {
    const drawerInner = cartDrawer.querySelector('li.mm-label.mm-listitem');
    if (drawerInner) {
      const topSellerDiv = createTopSellerElement();

      drawerInner.insertAdjacentElement('beforeBegin', topSellerDiv);
...
function checkCartDrawerAndToggle() {
  const cartDrawer = document.querySelector('form[id="cart"][method="post"]');
  if (cartDrawer) {
...
async function updateCartItemsUpsell() {
    const rows = document.querySelectorAll('li.mm-listitem');
    topProductState.cartItemsVariants = Array.from(rows).map(row => {
      const href = row.querySelector('a.cart__item')?.href;
      if (!href) return null;
      return href.split('variant=')[1];
    }).filter(id => id);

    // get quantity controls
    const quantityInputs = document.querySelectorAll('li.mm-listitem .cart__quantity');
    ...
    for (let i = 0; i < topProductState.cartItemsVariants.length; i++) {
      ...
      const allButtons = input.parentElement.querySelectorAll('span:not([data-set="true"])');
      if(allButtons) {
        allButtons.forEach(el => {
          if(!el.hasAttribute('data-set')) {
            if(el && el.classList.contains('minus')) {
            ...
            if(el && el.classList.contains('plus')) {
              const currentQty = parseInt(input.value, 10);
              ...
              el.addEventListener('click', async function (e) {
                ...
                e.target.style.disabled = true;
                input.value = newQty;
                await addTopSellerToCart(variantId, parseInt(data.volumeConfig.increment));
                window.reloadPageWithCartOpen('openCart', 'true');
              });
            }
