  ...
  document.body.addEventListener('click', async function(event) { 
    ...
    if (event.target.localName.toLowerCase() === 'label') {
      setTimeout(() => {
        ...
        if(productData != null && productData.hasOwnProperty('variants') && productData.variants.length > 0) {
          const idPrefix = 'ProductSelect-option-';
          var titleToFind = new Array();
          for(i = 0; i < 4; i++) {
            const selectedType = document.getElementById(`${idPrefix+i}`);
            if(selectedType) {
              const radioButton = selectedType.querySelector('input[type="radio"]:checked');
              titleToFind.push(radioButton.value);
            }
          }

          const productVariant = productData.variants.find(item => { return item.title == titleToFind.join(' / ') });
          if(productVariant) {
            const url = new URL(window.location.href);
            url.searchParams.set('variant', productVariant.id);
  ...
  window.productPageService = {
    createProductPageCustomPricing: function() {
      if (!window.productPageState.productOriginalFlag) {
        const qtyElement = `
          <div id="product-quantity-input-block" style="display: inline-flex; align-items: center; border: 1px solid #ccc; border-radius: 4px; width: fit-content;">
        ...
        `;
        window.productPageState.original.productOriginalPriceElement.innerHTML = priceElement;
        window.productPageState.new.productPriceElement = document.getElementById('ProductPrice');
        window.productPageState.new.productMSRPPriceElement = document.getElementById('ComparePrice');
...
function savingsButtonCssText() {
  return 'background: #000; color: white; padding: 5px; border-radius: 4px; font-weight: bold; margin-top: 4px; display: block; text-align: center; width: 100%; border: none; cursor: pointer;';
}
