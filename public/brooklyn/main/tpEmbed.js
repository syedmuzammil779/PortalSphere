...
function enableCartButtons() {
  const quantityButtons = document.querySelectorAll('.ajaxcart__qty-adjust');
...
function checkCartDrawerAndToggle() {
  setTimeout(() => {
    const cartDrawer = document.querySelector('#CartDrawer');
    if (cartDrawer && cartDrawer.classList.contains('js-drawer-open')) {
...
async function changeThisCartItem(input, type = 'increment') {
  disableCartButtons();
...
async function updateCartItemsUpsell() {
  ...
  const rows = ajaxCart.querySelectorAll('.ajaxcart__product')
  topProductState.cartItemsVariants = Array.from(rows).map(row => {
    const href = row.querySelector('.ajaxcart__product-image')?.href;
    if (!href) return null;
  ...
  for (let i = 0; i < topProductState.cartItemsVariants.length; i++) {
    ...
    const setInputConstraints = (input, config) => {
      ...
      input.parentElement.querySelector('.ajaxcart__qty--minus').addEventListener('click', async function (e) { 
        e.preventDefault();
        e.stopImmediatePropagation();
        await changeThisCartItem(input, 'decrement'); 
      }, true);
      input.parentElement.querySelector('.ajaxcart__qty--plus').addEventListener('click', async function (e) {  
        e.preventDefault();
        e.stopImmediatePropagation();
        await changeThisCartItem(input, 'increment');
      }, true);
    };
