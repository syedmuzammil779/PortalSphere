...
$(document).ready(async function () {
  ...
  $(document).on('click', '.remove_line_item', function (e) {
    e.preventDefault();
    var el = $(this);
    fetch('/cart/change.js', {
      ...
      body: JSON.stringify({
        id: el.data('variantid').toString(), // Replace with the actual line item ID you want to remove
        quantity: 0
      })
    })
  ...
  var quantitySelectorHTML = null;
  quantitySelectorHTML = `Min. ${min} • Increments of ${step}`;
  $(quantitySelector).parent().parent().parent().append(`<p style="margin-top:20px">${quantitySelectorHTML}</p>`)
  ...
  newPlusBtn.click((e) => {
    ...
    updatePriceFromConfig(priceCfg, $('.product-block--price'), $(quantitySelector).val())
  })
  newMinusBtn.click((e) => {
    e.preventDefault();
    let inc = parseInt($(quantitySelector).attr("step"))
...
const updatePriceFromConfig = (priceCfg, target, currentQuantity) => {
  ...
  target.html(`${msrp} <b>${currencySymbol}${price} ${currencyCode}</b> `)
}
...
const updatePrice = (priceCfg, target, currentQuantity) => {
  ...
  const { currencySymbol, price, currencyCode, originalPrice } = currentPriceSlab
  const msrp = `<span style="font-size: 1rem"><s>MSRP ${currencySymbol}${originalPrice} ${currencyCode}</s></span>`
  target.parent().parent().html(`${msrp} <b>${currencySymbol}${price} ${currencyCode}</b> `)
}
 ...
 *
 */
async function populateHomePagePrices(handleArray) {
    ...
    if (response.count > 0) {
      for (var i in response.data) {
        ...
        if (priceConfig.length > 1) {
          const { price: highestPrice, currencySymbol, currencyCode, percentage } = priceConfig[0];
          ...
          const saveBadge = $(priceContainer).parent().parent().find('.grid-product__tag--sale');
          if(saveBadge) {
            $(saveBadge).html(`Save ${percentage}%`)
          }
        } else if (priceConfig.length == 1) {
          const { price, originalPrice, currencySymbol, currencyCode, percentage } = priceConfig[0];
...
async function addTopupButtons(rowData) {
  ...
  rowData.map(function () {
    ...
    const productLink = $(this).find('a.cart__product-name');
    const params = new URLSearchParams(productLink.attr('href').split("?")[1]);
    const variantId = params.get("variant");
    plusBtn.parent().parent().parent().find('.cart__item--name').append(`<span class="remove_line_item" data-variantid="${variantId}" style="float:right; cursor:pointer">Remove</span>`);
