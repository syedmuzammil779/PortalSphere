...
async function initializeProductState() {
  return new Promise((resolve) => {
    ...
    for (const selector of variantPickerSelectors) {
      const variantElement = document.querySelectorAll(selector);
      if (variantElement) {
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  if(window.productPageState.original.productOriginalVariantPicker.length > 0) {
    $(window.productPageState.original.productOriginalVariantPicker).click(function () {
      setTimeout(() => {
        location.reload();
      }, 500);
    })
