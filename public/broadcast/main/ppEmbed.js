...
(async function () {
  ...
  `;
    ...
    if ((isB2B || isHybrid) && isCustomerLoggedIn && customerTag) {
        const currentPage = window.location.pathname;
        if (currentPage.includes('/products/')){
            $("div.product__submit__item.product__submit__item--inline").css({
              "display": "flex",
              "flex-direction": "column",
              "gap": "10px"
            })
            $("div.product__submit__item.product__submit__item--inline .btn__price").css("display", "none")
    ...
    // get products cards and pricing
      ...
      $("div.product-item__price__holder span.price span.new-price").text("Loading...")
      try{
        $("div.product-item__price__holder span.price span.old-price").css("display", "none")
      ...
      pricingData.data.map((item) => {
        ...
        if (priceConfig.length > 1) {
          const { price: highestPrice, currencySymbol } = priceConfig[0];
          const { price: lowestPrice } = priceConfig[priceConfig.length - 1];

          const content = `
            From ${currencySymbol}${lowestPrice} - ${currencySymbol}${highestPrice}
          `
          $(`div.product-item:has(a[href*='/products/${name}']) div.product-item__price__holder span.price span.new-price`).html(content)
        }
        else if (priceConfig.length == 1) {
          const { price, originalPrice, currencySymbol } = priceConfig[0];
          const content = `${currencySymbol}${price} <s>MSRP ${currencySymbol}${originalPrice}</s>`
          $(`div.product-item:has(a[href*='/products/${name}']) div.product-item__price__holder span.price span.new-price`).html(content)
        }
      })
    } catch (error) {
    ...
    const rows = $('cart-items div.cart__item')
    ...
    if (target) {
      const observer = new MutationObserver(function (mutationList) {
        mutationList.forEach((mut) => {
          const hasAll = ["drawer__items"].every(cls => mut.target.classList.contains(cls))
          if(hasAll){
            const rows = $('cart-items div.cart__item')
            addTopupButtons(rows)
          }
...
function getProductsCards() {
  let handleArr = []
  const itemLinks = document.querySelectorAll("a.product-link[aria-label]")
...
async function addTopupButtons(rowData) {
  ...
  rowData.map(function () {
    let addButton
    const container = $(this).find('div.cart__quantity-counter');
    ...
    if (foundButton.length < 1) {
      addButton = $('<button type="button" class="to-next-interval"></button>').css({
        ...
        $(this).find("button.cart__quantity-plus").trigger("click")
      })
    ...
    const addBtn = $(this).find("button.cart__quantity-plus")
    const minusBtn = $(this).find("button.cart__quantity-minus")

    const variantId = $(this).attr("data-item").split(":")[0]
  });
  ...
  intervalInfo.map(({ name, amountToAdd, benefit, benefitType, currencySymbol }) => {
    let button = $(`div.cart__item:has(a[href^="/products/${name}"]) button.to-next-interval`)
...
function calculateIntervalData(volumeData) {
  ...
  volumeData.map((item) => {
    ...
    const quantityInput = row.find('input.cart__quantity-field')
