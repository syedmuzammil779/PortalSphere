...
(async function () {
        ...
        if (currentPage.includes('/products/')){
            ...
            if (window.productPageState.new.productPriceElement) {
                ...
                try {
                    ...
                    console.log('error in line 198');
                    console.log(error.message);    
                }
                
                window.productPageState.new.productQuantityElement.insertAdjacentElement('afterend', quantityInfo); 
                document.querySelectorAll('.product-form__buttons').forEach(el => {
                    el.style.display = 'block';
                });

                document.querySelectorAll('button[type="submit"]').forEach((btn, index) =>  {
                    if(btn.classList.contains('product-form__submit')) {
    ...
    setTimeout(async () => {
        ...
        .then(results => {
            results.forEach(({ productCard, pricing_1 }) => {
                if (pricing_1 && pricing_1.priceConfig.length > 0) {
                    // const priceDisplay = productPriceSelectors
                    // .map(selector => productCard.cardElement.querySelector(selector))
                    // .find(el => el);

                    var findInputElement = document.querySelector('input[type="hidden"][name="id"][value="'+productCard.productId+'"]');
                    var priceDisplay = findInputElement.parentElement.parentElement.querySelector('.price__container');

                    if(!priceDisplay) {
                        priceDisplay = findInputElement.parentElement.parentElement.parentElement.querySelector('.price__container');
                    }

                    if (priceDisplay) {
                        const { currencySymbol, price, originalPrice } = pricing_1.priceConfig[0];
                        ...
                        console.log(`Price display element not found for product card:`, productCard);
                    }
                } else {
                    console.log(`Pricing data is missing or empty for product card:`, productCard);
                }
            });

            var allSoldOutCards = document.querySelectorAll('.product-card__sold-out');
            if(allSoldOutCards) {
                allSoldOutCards.forEach(card => {
                    const priceEl = card.querySelector('.price__container');
                    if (priceEl) {
                        priceEl.innerHTML = 'Sold Out!';
                    }
                });
            }
        })
...
function extractProductId(cardElement) {
    try {
        ...
        if (window.location.pathname.includes('/cart')) {
            const swipingProduct = cardElement.querySelector('.product-card__link');
            if (swipingProduct.length > 0) {
                const link = swipingProduct.href;
                if (link) {
        ...
        const productIdInp = cardElement.querySelector('input[name="id"]');
