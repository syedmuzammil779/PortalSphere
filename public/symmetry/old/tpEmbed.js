...
(function() {
  ...
  function insertTopSeller() {
    ...
    const cartDrawer = document.querySelector('cart-drawer');
    const miniCartInner = cartDrawer?.querySelector('.cart-drawer__content');
    const cartFooter = cartDrawer?.querySelector('.cart-drawer__footer');

    if (!cartDrawer || !miniCartInner) {
      ...
      if (cartFooter && miniCartInner.contains(cartFooter)) {
      ...
      if (topProductState.addToCartTopProduct) {
        topProductState.addToCartTopProduct.onclick = async (e) => {
          ...
          setTimeout(() => {
          waitForDrawerAndInsertTopSeller();
        }, 50);
...
async function addTopSellerUsingUpdateCart(variantId, quantity, properties = {}) {
  ...
  if (priceInfo) {
   ...
   async function updateCartItemsUpsell() {
      const cartItemElements = document.querySelectorAll('.cart-item-list__body .cart-item');

    topProductState.cartItemsVariants = Array.from(cartItemElements).map(item => {
        return item.querySelector('.quantity__input')?.dataset.variantId || null;
      }).filter(id => id);

    topProductState.cartQuantityInputs = Array.from(cartItemElements).map(item => {
      return item.querySelector('.quantity__input');
    }).filter(input => input);
  ...
  for (let i = 0; i < topProductState.cartItemsVariants.length; i++) {
    ...
    if (input) {
      input.addEventListener('change', async () => {
        ...
        try {
          ...
          setInputConstraints(input, updatedData);
    ...
    const container = itemElement.querySelector('.cart-item__item-discounts') || itemElement;
