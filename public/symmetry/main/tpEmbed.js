...
function insertTopSeller() {
    ...
    if (cartDrawer && !cartDrawer.querySelector('#top-product-embed')) {
        const drawerInner = cartDrawer.querySelector('.cart-drawer__content');
        if (drawerInner) {
            const topSellerDiv = createTopSellerElement();

            const drawerFooter = drawerInner.querySelector('div[data-merge="cross-sells"]');
