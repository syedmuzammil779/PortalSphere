...
function initializeComplementaryProductState() {
    // Ensure objects exist before setting their properties
    window.productPageState = window.productPageState || {};
    window.complementaryProductState = window.complementaryProductState || {};
...
async function waitForCustomerService() {
    while (typeof window.customerService === 'undefined') {
        await new Promise(resolve => setTimeout(resolve, 100));
    }
}

async function getCustomerTag() {
    await waitForCustomerService();  // Ensure it's defined
    const customerId = cpConfig.customerId;
    const tag = await window.customerService.getCustomerTag(
        cpConfig.appDomain,
        cpConfig.shopDomain,
        cpConfig.apiKey,
        cpConfig.timestamp,
