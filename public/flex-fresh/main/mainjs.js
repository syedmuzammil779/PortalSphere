...
function refreshCartDrawerAndCountNonDestructive() {
  const cartDrawer = document.querySelector('#theme-ajax-cart .ajax-cart--drawer');
...
window.bindCartDrawerQuantityButtons = function () {
  document.querySelectorAll('quantity-input .quantity__button').forEach((button) => {
    button.addEventListener('click', async (event) => {
      ...
      const variantId = button.closest('.ajax-cart__product')?.getAttribute('data-cart-item')?.split(':')[0];
...
window.reloadCartDrawer = async function reloadCartDrawer() {
  try {
    ...
    const drawer = document.querySelector('#theme-ajax-cart .ajax-cart--drawer');
...
function toggleCartDrawerQuantityInteractivity() {
  ...
  const quantityInputs = document.querySelectorAll('.ajax-cart__product .quantity-input');
  const quantityButtons = document.querySelectorAll('.ajax-cart__product .quantity-plus, .ajax-cart__product .quantity-minus');
    ...
    addProductToCart: async function (variantId, quantity, properties = {}) {
      try {
        ...
        const themeSectionMap = {
          "Flex": ['theme-ajax-cart', 'header'],
          "default": ['theme-ajax-cart', 'header']
        };
        ...
        await refreshCartDrawerSections(); // 🔄 Update drawer DOM
         waitForMiniCartInnerAndInsert();   // 👀 Observe drawer content load
        // waitUntilCartUpdated(variantId, quantity); // ✅ Wait for backend update
    ...
    updateProductToCart: async function (variantId, quantity, properties = {}) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          await refreshCartDrawerSections(); // 🔄 Update drawer DO
  ...
  document.body.addEventListener('click', async function(event) {
    ...
    if (clickedButton && clickedButton.name === 'add' && clickedButton.type === 'submit') {
      ...
      try {
       ...
       await refreshCartDrawerSections();           // ✅ wait for fresh drawer
  ...
  window.productPageService = {
    createProductPageCustomPricing: function() {
      if (!window.productPageState.productOriginalFlag) {
        ...
        const oldQuantityElement = document.querySelector('.purchase-details__quantity');
        ...
        if (!document.getElementById('product-quantity-input-block')) {
          ...
          const insertAfterElement = document.querySelector('.purchase-details__buttons');
...
if (!window.topSellerPopupService) {
  ...
  // Generate popup HTML string once
  const popupHTML = window.topSellerPopupService.generateTopSellerPopup();

  // Find all drawer footers and their button wrappers
  const drawerFooters = document.querySelectorAll('.ajax-cart__details-wrapper');
  
  if (!drawerFooters.length) {
    // No drawer wrappers found — append one popup to body as fallback
    ...
    console.warn('[Top Seller Patch] No drawer wrappers found, appended popup to body');
    
    this.attachCloseLogic(popupDiv);
    return [popupDiv];
  }

  const popups = [];

  drawerFooters.forEach((drawerFooter, index) => {
    // Find button wrapper inside current drawer footer
    const buttonWrapper = drawerFooter.querySelector('.ajax-cart__button-wrapper');

    // Create a new popup element per drawer to avoid duplicate IDs
    const popupDiv = document.createElement('div');
    // Give each popup a unique ID or remove ID to avoid duplicates
    popupDiv.id = `top-seller-popup-${index}`;
    popupDiv.className = 'popup-seller';
    popupDiv.style.display = 'none';
    popupDiv.style.justifyContent = 'center';
    popupDiv.style.alignItems = 'center';
    ...
    if (buttonWrapper) {
      // Insert popup before the checkout button
      drawerFooter.insertBefore(popupDiv, buttonWrapper);
      console.debug(`[Top Seller Patch] Popup inserted above checkout button in drawer #${index}`);
    } else {
      // If button wrapper missing, append popup at end of drawerFooter
      drawerFooter.appendChild(popupDiv);
      console.warn(`[Top Seller Patch] Button wrapper not found in drawer #${index}, appended popup at end`);
    }

    // Attach close logic for this popup
    this.attachCloseLogic(popupDiv);

    popups.push(popupDiv);
  });

  return popups; // Return array of popup elements inserted
    },
    // Helper method to attach close button handlers to a popup element
    attachCloseLogic: function(popupDiv) {
      ...
      if (nothanksBtn) nothanksBtn.onclick = closePopup;
...
window.topProductEmbedUtils = {
  ...
  findCartDrawerFooter: function() {
    const cartDrawerSelectors = [
      ".header-cart #theme-ajax-cart .ajax-cart__details-wrapper",
    ];
...
document.addEventListener('DOMContentLoaded', async function() {  
 ...
 // Variant switch fix for themes not updating URL (e.g., Flex, Motion-Cal)
...
if (window.location.pathname.includes('/cart') || document.querySelector('#theme-ajax-cart .ajax-cart--drawer')) {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!recommendationsExist) {
     const fallbackTopSeller = generateTopSellerBlock(); // from global function
      ...
      console.log("cartDrawerFooter",cartDrawerFooter);
...
function openCartDrawer() {
  ...
  const isOpen = cartDrawer.classList.contains('is-visible') || cartDrawer.getAttribute('aria-hidden') === 'false';
...
function waitForMiniCartInnerAndInsert(maxWait = 1000) {
  ...
  const target = cartDrawer.querySelector('#ajax-cart__content');
  ...
  setTimeout(() => {
    ...
    insertTopSeller();
    const cartFooter = cartDrawer.querySelector('.header-cart #theme-ajax-cart .ajax-cart__details-wrapper');
      // ⬇ Show popup + hide footer
      if (cartFooter) cartFooter.style.display = 'none';
    }, maxWait);
}

async function waitUntilCartUpdated(targetVariantId, expectedQuantity, maxWait = 3000, pollInterval = 250) {
  const start = Date.now();
  ...
  return new Promise((resolve) => {
    async function check() {
      try {
        ...
        console.error('[TopSeller] Error fetching cart:', err);
        // You may want to resolve(false) or continue retrying depending on error type
      }

      if (Date.now() - start < maxWait) {
        setTimeout(check, pollInterval);
      } else {
        console.warn('[TopSeller] Cart did not update in time — resolving anyway.');
        resolve(false);
      }
    }
...
function waitForCartDrawerItems() {
  return new Promise((resolve) => {
    const container = document.querySelector('#theme-ajax-cart .ajax-cart__list');
    if (!container) {
      resolve();
      return;
    }
...
// Main call
async function openDrawerThenInsertTopSeller() {
...
function waitForDrawerContent() {
  return new Promise((resolve) => {
    const container = document.querySelector('#theme-ajax-cart .ajax-cart__list');
    if (!container) {
      console.warn('[TopSeller] Drawer container not found.');
      resolve();
      return;
    }
...
// ✅ Fully updated drawer+header refresher
async function refreshCartDrawerSections() {
  try {
    // 🔒 Disable quantity inputs during update
    disableCartQuantityInputs();

    if (window.PXUTheme?.jsAjaxCart?.updateView) {
      await window.PXUTheme.jsAjaxCart.updateView(); // ✅ redraw cart drawer
      console.debug('[TopSeller] PXUTheme cart drawer updated via updateView()');
    } else {
      console.warn('[TopSeller] PXUTheme.jsAjaxCart.updateView not available.');
    }

    // Optional: update header count manually
    const cart = await window.cartService.getCart();
    document.querySelectorAll('[data-bind="itemCount"]').forEach(el => {
      el.textContent = cart.item_count;
    });

    // Rebind interactivity (quantity logic, upsells, etc.)
    updateCartItemsUpsell?.();
  

  } catch (err) {
    console.error('[TopSeller] Failed to refresh PXUTheme cart drawer:', err);
  } finally {
    enableCartQuantityInputs(); // 🔓 Done
  }
}
...
function disableCartQuantityInputs() {
  document.querySelectorAll('input.quantity-input, .quantity-minus, .quantity-plus').forEach(el => {
    el.disabled = true;
...
async function refreshCartDrawerAfterUpdateQuantity() {
  ...
  try {
    ...
    if (sectionHtml[sections[0]]) {
      ...
      const existingDrawer = document.querySelector('#theme-ajax-cart .ajax-cart--drawer');
