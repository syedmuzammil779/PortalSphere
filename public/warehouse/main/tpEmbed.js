...
(function() {
  ...
  if (tpConfig.enableTopProducts === 'true') {
    ...
    setInterval(checkCartDrawerAndToggle, 2000);
...
async function insertTopSeller() {
  const cartDrawer = document.querySelector('.mini-cart');
  var customerTag = await getCustomerTag();
  if (cartDrawer && !cartDrawer.querySelector('#top-product-embed')) {
    const drawerInner = cartDrawer.querySelector('.mini-cart__button-container');
    if (drawerInner) {
      const topSellerDiv = createTopSellerElement();
      
      drawerInner.insertAdjacentElement('beforeend', topSellerDiv);
      ...
      setTimeout(() => {
        const scrollContainer = document.querySelector('.mini-cart__line-item-list');
        if (scrollContainer) {
      ...
      topProductState.addToCartTopProduct.onclick = async (e) => {
        ...
        document.querySelector('.header__action-item-link').click();
...
async function fetchTopSeller() {
  ...
  try {
    ...
    if(data != null && data.hasOwnProperty('productVariantId')) {
...
function updateTopSellerUI(data, pricing) {
  ...
  if (topprodembed) {
    ...
    if(data.productInfo.image) {
      ...
      img.onerror = function() { console.error('Failed to load image:', data.productInfo.image); };
    }
...
async function recordButtonClick(buttonType, tag, operation) {
  await window.customerService.recordButtonClick(tpConfig.appDomain, tpConfig.shopDomain, tpConfig.apiKey, tpConfig.timestamp, tpConfig.hmac, tpConfig.customerId, tag, buttonType, operation);
}

async function updateCartItemsUpsell() {
  const rows = document.querySelectorAll('.mini-cart__line-item');
  if(!rows) return;

  let cartItemIds = new Array();
  var href, variant_id;
  rows.forEach(el => {
    href = el.querySelector('a.mini-cart__product-title').href;
    variant_id = href.split('variant=')[1];
    if(variant_id)
      cartItemIds.push(variant_id);
  })
  
  topProductState.cartItemsVariants = cartItemIds;

  // get quantity controls
  const quantityInputs = document.querySelectorAll('.mini-cart__quantity');
  topProductState.cartQuantityInputs = Array.from(quantityInputs).map(container => {
    return container.querySelector('input.quantity-selector__value');
  }).filter(input => input);
  ...
  for (let i = 0; i < topProductState.cartItemsVariants.length; i++) {
    ...
    savingsButton.onclick = async (e) => {
      ...
      await recordButtonClick('cart_line_upsell', customerTag, JSON.stringify({"diff": diff, variantId: newVariantId}));
    ...
    container.querySelector('.mini-cart__product-info').appendChild(savingsButton);
  }
}