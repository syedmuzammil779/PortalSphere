...
(async function () {
  ...
  `;
    ...
    if ((isB2B || isHybrid) && isCustomerLoggedIn && customerTag) {
        ...
        if (currentPage.includes('/products/')){
            try {
                ...
                const { volumeConfig, priceConfig } = data
                if(volumeConfig != null && volumeConfig.hasOwnProperty('minimum') && volumeConfig.hasOwnProperty('increment') && volumeConfig.hasOwnProperty('maximum')) {
                    const { minimum, increment, maximum } = volumeConfig
                    const min = minimum;
                    const inc = increment;
                    const max = maximum === Number.MAX_SAFE_INTEGER ? '∞' : maximum;
                    if (!maximum || maximum == Number.MAX_SAFE_INTEGER) {
                        $("gift-card-recipient-form").after(`<span>Min. ${min} &#x2022; Increments of ${inc}</span>`);
                    } else {
                        $("gift-card-recipient-form").after(`<span>Min. ${min} &#x2022; Max ${max} &#x2022; Increments of ${inc}</span`);
                    }
                    $("product-form-component[data-product-url] input[ref='quantityInput']").attr({
                      "min": minimum,
                      "max": maximum,
                      "step": increment,
                      "value": minimum,
                  ...
                  $("product-form-component[data-product-url] button.button.quantity-plus").attr("on:click", "")
                  $("product-form-component[data-product-url] button.button.quantity-plus").click((e) => {
                    e.stopPropagation()
                    const quantitySelector = $("product-form-component[data-product-url] input[ref='quantityInput']")
                    quantitySelector[0].stepUp()
                    const quantity = parseInt(quantitySelector.val())
                    const currPrice = priceConfig.find((item) => item.maxQuantity >= quantity && parseInt(item.quantity) <= quantity)
                    const { currencySymbol, currencyCode, price } = currPrice
                    const content = `
                        ${currencySymbol}${price} ${currencyCode}
                    `
                    $("div.group-block-content product-price span.price").text(content)
                  })
                  $("product-form-component[data-product-url] button.button.quantity-minus").attr("on:click", "")
                  $("product-form-component[data-product-url] button.button.quantity-minus").click((e) => {
                    e.stopPropagation()
                    const quantitySelector = $("product-form-component[data-product-url] input[ref='quantityInput']")
                  ...
                  const { currencySymbol, currencyCode, price } = priceConfig[0]
                  const content = `
                      ${currencySymbol}${price} ${currencyCode}
                  `
                  $("div.group-block-content product-price span.price").text(content).css({
                    "font-size": "1.4rem"
                  })
                }
            } catch(err) {
        ...
        const target = document.querySelector('div.cart-drawer__inner');
        ...
        if (target) {
          const observer = new MutationObserver(function (mutationList) {
            mutationList.forEach((mut) => {
              const hasAll = ['cart__total-value', 'cart-secondary-typography'].every(cls => mut.target.classList.contains(cls))
              if(hasAll && mut.attributeName === "value"){
                const rows = $('tbody[role="rowgroup"] tr')
                addTopupButtons(rows)
              }
            })
    ...
    try {
      $("div.product-card__content div[ref='priceContainer'] span.price").text("Loading...")
      $("div.product-card__content div[ref='priceContainer'] span.compare-at-price").css("display", "none")
      ...
      pricingData.data.map((item) => {
        ...
        if (priceConfig.length > 1) {
          ...
          `
          $(`div.product-card__content:has(a[href*='/products/${name}']) product-price span.price`).html(content)
        }
...
function getProductsCards() {
  let handleArr = []
  const itemLinks = document.querySelectorAll("a[ref^='productTitleLink']")
...
async function addTopupButtons(rowData) {
  ...
  rowData.map(function () {
    let addButton
    const container = $(this).find('td.cart-items__quantity');
    const foundButton = container.find('td.top-up-button button[type="button"].to-next-interval')
    if (foundButton.length < 1) {
      addTd = $('<td class="top-up-button"></td>')
      addButton = $('<button type="button" class="to-next-interval"></button>').css({
        ...
        $(this).find("button.quantity-plus").trigger("click")
      })
      addTd.append(addButton)
      container.after(addTd)
    }
    else {
    ...
    const addBtn = $(this).find("button.quantity-plus")
    const minusBtn = $(this).find("button.quantity-minus")

    addBtn.attr("on:click", "")
    minusBtn.attr("on:click", "")

    const variantId = $(this).attr("data-key").split(":")[0]
    if (!addedListeners.includes(variantId)) {
      addedListeners.push(variantId)
  ...
  intervalInfo.map(({ name, amountToAdd, benefit, benefitType, currencySymbol }) => {
    let button = $(`tr.cart-items__table-row:has(a[href^="/products/${name}"]) button.to-next-interval`)
...
function calculateIntervalData(volumeData) {
  ...
  volumeData.map((item) => {
    ...
    const row = $(`tr.cart-items__table-row:has(a[href^="/products/${productVariantHandle}"])`).first()
    const quantityInput = row.find('input[ref="quantityInput"]')
