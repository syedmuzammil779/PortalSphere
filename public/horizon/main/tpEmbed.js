...
function insertTopSeller() {
    const cartDrawer = document.querySelector('dialog.cart-drawer__dialog');
    console.log(cartDrawer)
    if (cartDrawer && !cartDrawer.querySelector('#top-product-embed')) {
        const drawerInner = cartDrawer.querySelector('div.cart-drawer__inner');
        if (drawerInner) {
            const topSellerDiv = createTopSellerElement();

            const drawerFooter = drawerInner.querySelector('.cart-drawer__summary');
            if (drawerFooter) {
                drawerFooter.prepend(topSellerDiv);
            }
...
async function updateCartItemsUpsell() {
    const cartItemsTable = document.querySelectorAll('.cart-items__table[role="table"]');
    const rows = cartItemsTable[0].querySelectorAll('tbody > tr')
    topProductState.cartItemsVariants = Array.from(rows).map(row => {
        const href = row.querySelector('td a.cart-items__title')?.href;
        if (!href) return null;
        return href.split('variant=')[1];
    }).filter(id => id);

    // get quantity controls
    const quantityInputs = document.querySelectorAll('quantity-selector-component');
